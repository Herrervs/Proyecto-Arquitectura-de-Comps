...
DIR_BUF EQU     $5000           * Direccion de memoria libre para el buffer temporal
...
BUCLE_ECO:
        ...
        * Apilamos los 3 parametros de derecha a izquierda: tamano, descriptor y direccion
        MOVE.W  #TAMANO,-(A7)   * Apila el tamano maximo a leer
        MOVE.W  #0,-(A7)        * Apila el descriptor: 0 = Linea A
        MOVE.L  #DIR_BUF,-(A7)  * Apila la direccion de inicio del buffer
        BSR     SCAN            * Llama a la subrutina de lectura
        ADD.L   #8,A7           * Limpiamos la pila

        * En este punto, D0 contiene el numero real de caracteres leidos
        * Si no se ha leido nada (D0 = 0) o hubo error, volvemos al inicio
        CMP.L   #0,D0
        BLE     BUCLE_ECO       * Si D0 <= 0, repite el bucle

        * --- FASE DE ESCRITURA (PRINT) ---
        * Reutilizamos el tamano exacto que devolvio D0 para imprimir solo lo leido
        MOVE.W  D0,-(A7)        * Apila el tamano a imprimir (2 bytes, desde D0)
        MOVE.W  #0,-(A7)        * Apila el descriptor: 0 = Linea A
        MOVE.L  #DIR_BUF,-(A7)  * Apila la direccion del buffer
        BSR     PRINT           * Llama a la subrutina de escritura
        ADD.L   #8,A7           * Limpiamos la pila

        BRA     BUCLE_ECO       * Bucle infinito para probar el eco continuamente
        
        BREAK                   * Instruccion de parada (inalcanzable por BRA)
